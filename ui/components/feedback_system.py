...
class FeedbackSystem:
    ...
    def render_feedback_tab(self):
        ...
        # If no comparison report exists, this indicates the workflow didn't complete properly
        # if not comparison_report:
        #     st.warning("Comparison report not available. The workflow may not have completed properly.")
        #     comparison_report = self._generate_fallback_report(state, latest_review)
    ...
    def _generate_fallback_report(self, state, latest_review) -> str:
        """Generate a fallback comparison report if workflow didn't generate one."""
        try:
            if not latest_review or not latest_review.analysis:
            ...
            report = f"""# {t('review_feedback')}
                ...
                - {t('review_attempts')}: {len(state.review_history) if state.review_history else 0}
